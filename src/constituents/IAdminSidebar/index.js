...
import './styles.css';
const AdminSidebar = ({isOpen,setIsOpen, className, style, login_task_all}) => {
        ...
        <>
            ...
            <div id="sidebar" name="sidebar" className='sidebar-large over-height'>
                <div>
                    ...
                    <NavLink to="/start-new-role-play" activeClassName={`current`} id="sidebar_link" name="sidebar_link"><img src={logo} className="mw-100" alt="logo"  id="sidebar_icon" name="sidebar_icon"/></NavLink> <h6 className="" id="sidebar_header" name="sidebar_header">{t('sidebar.recruiter_training_ai')}</h6>
                    </div>
                    ...
                    <ul className="sidebar-menu" ref={elementRef}>
                    {
                        adminSidebarList.map((menu, index) => {
                            if(!removeSidebar.includes(menu.name)){
                                return <li key={index} id={menu.name} name={menu.name}><NavLink id={`link_${menu.name}`} name={`link_${menu.name}`} to={menu.url}  activeClassName={`current`}><img id={`icon_${menu.name}`} name={`icon_${menu.name}`} src={menu.icon} alt="sidebar icon"/><span id={`text_${menu.name}`} name={`text_${menu.name}`}>{t(menu.text)}</span></NavLink></li>
                            }
                        })
                    }
                    </ul>
