...
const VideoChatPage = () => {
  ...
  const goBack = () => {
    if(state.tab)
    {
      history.replace(`/admin/create/${state.tab}`);
    }else{
  ...
  const confirmSaveProcess = (type) => {
    if (type == "apply") {
        ...
        if (v.scriptLineList.length) {
          v.scriptLineList.map((v2, k2) => {
            let scriptLine = {
              ...
              scriptPersonaAction: v2.scriptLinePersonaAction,
            };
            let keywordArray = [];
      ...
      saveAndUpdateMatain(formatChange);
    } else {
      setShowConfirmOpen(false);
    }
  };
  const saveAndUpdateMatain = async (formatChange) => {
    const response = await saveAndUpdateMatainSection(formatChange, sectionId);
                        ...
                        {item.processReference.map((item2, index2) => {
                          return (
                            ...
                            >
                              <div className="mb-4">
                                {vProcessToken ? (
                                  <a href={`${item2.referencePath}?${vProcessToken}`} target="_blank">
                                    <img
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="d-flex flex-wrap mb-3 align-items-center" 
                          ...
                          {item.keywords.map((item2, index2) => {
                            return (
                              <KeywordTag
                                key={index2}
                                title={item2.keyWord}
