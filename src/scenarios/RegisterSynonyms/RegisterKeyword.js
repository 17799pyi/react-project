...
import { deleteKeywords, postKeywords } from "../../request/api";
        ...
        <Row>
          <Col xl={synonymsColGrid()}>
            {synonyms.map((synonym, index) => {
              return (
                <DeleteKeywordButton
                  title={synonym}
                  key={index}
                  className={`mr-3 align-items-center d-inline-block mb-2 font-gray ${classes.height_64}`}
            ...
            {isEditKeyword && (
              <GeneralTextbox
                placeholder=""
                className={`${classes.synonym_text_box}`}
                value={newSynonym}
                autoFocus={true}
